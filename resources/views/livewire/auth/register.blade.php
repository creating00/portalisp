...
<div class="flex min-h-screen w-full bg-brand-dark">
    ...
    <div class="w-full lg:w-[45%] flex flex-col p-8 lg:p-16 xl:p-24 overflow-y-auto">
        ...
        <x-auth.header title="{{ __('Create Account') }}" subtitle="{{ __('Join the network.') }}" is-dark="true" />
        ...
        <div class="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
            ...
            <form wire:submit.prevent="register" class="space-y-5">
                ...
                <x-auth.input-dark label="{{ __('Email Address') }}" type="email" id="email"
                    placeholder="name@company.com" icon="mail" model="email" />
                @error('email')
                    <span class="text-red-400 text-xs">{{ $message }}</span>
                @enderror

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-auth.input-dark label="{{ __('Password') }}" type="password" id="password"
                        placeholder="••••••••" icon="lock" model="password" />

                    <x-auth.input-dark label="{{ __('Confirm Password') }}" type="password" id="password_confirmation"
            ...
            <p class="mt-8 text-center text-slate-400 text-sm">
                ...
                <a class="text-primary-orange font-bold hover:underline" href="/login">{{ __('Log In') }}</a>
            </p>
        </div>
